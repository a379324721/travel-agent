...
- **差旅全流程闭环**：身份识别 → 行程规划 → 差标校验 →（超标审批）→ 订票落库 → 一键报销；供应商库存、OA 审批、员工目录当前为 mock 适配层，接口留好替换真实系统
- **对话与工具调用**：意图路由（闲聊不走工具、政策问题强制查制度）+ 工具注册中心统一调度 10 个业务工具
- **RAG**：文档分块（ETL）+ 嵌入写入 Milvus；对话中自动检索差旅制度原文作答
- **对话记忆**：Redis 会话持久化（按 `session_id`）+ token 预算窗口 + 超长自动摘要
- **工程化**：健康检查（Redis / PostgreSQL / Milvus）、运行指标、熔断器、结构化日志、OpenTelemetry
